...
class ServiceBroker {
   constructor(config, tokenService) {
      ...
      this.config = config;
      this.elvisService = new ElvisService(config.elvisTemplate + "&" + config.nameValuePair, tokenService);
      this.fsdfService = new FsdfService(config.postProcessingUrl, tokenService);
