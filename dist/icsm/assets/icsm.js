...

{
	...
	//'common.panes',
	'common.storage', 'common.templates', 'explorer.config', 'explorer.confirm',
	// 'ed.download',
	...
	// Alternate list
	'elvis.header', 'elvis.results', 'elvis.reviewing', 'icsm.mapevents', 'icsm.select', 'icsm.splash', 'icsm.state', 'icsm.layerswitch', 'icsm.templates', 'elevation.toolbar', 'icsm.view'])
...
{
   (function () {
      var decorateDownloadables = function decorateDownloadables(downloadables) {
      ...
      }]).factory("boundsService", ['$http', '$q', '$rootScope', '$timeout', 'configService', function ($http, $q, $rootScope, $timeout, configService) {
            ...
            if (result && result.code) {
               switch (result.code) {
                  case "oversize":
                     $timeout(function () {
                        send("", "clear");
                        send("The selected area is too large to process. Please restrict to approximately " + "4 degrees square.", "error");
...
{
   angular.module("products.bbox", []).directive("productsBboxButton", ['productsBboxService', function (productsBboxService) {
      return {
      ...
      service.show = function (data) {
         if (!data.boundsLayer) {
            var bounds = productsMapUtilsService.bboxToBounds(data.bbox);
...

{
   (function () {
      ...
      angular.module("icsm.product", ["products.bbox", "product.download"]).directive("icsmProduct", [function () {
         return {
            scope: {
               dataset: "="
            },
...
{
   (function () {
      angular.module("icsm.products", ["icsm.product"]).directive("icsmProducts", ['productsConfigService', function (productsConfigService) {
         return {
            restrict: "AE",
...

{
   (function () {
      ...
      angular.module("elvis.results", ['elvis.results.continue', 'icsm.subtype', 'icsm.unreleased']).directive('icsmOrgHeading', [function () {
         return {
                  ...
                  if (value) {
                     modalInstance = $uibModal.open({
                        ...
                        controller: ['$scope', '$uibModalInstance', 'products', function ($scope, $uibModalInstance, products) {
                           ...
                           $scope.setPage = function (page, pagesize) {
                              $scope.page = $scope.products ? $scope.products.filter(function (product) {
                                 return product.selected;
            ...
            startExtract: function startExtract() {
               ...
               return configService.getConfig("processing").then(function (config) {
                  var postData = convertFlatToStructured(listService.products.filter(function (product) {
                     return product.selected && !product.removed || product.type === "Unreleased Data";
                  }));
