...
{
	angular.module("icsm.bounds", ["icsm.message"]).directive('icsmBounds', ['icsmMessageService', 'boundsService', function (icsmMessageService, boundsService) {
		return {
			restrict: 'AE',
			link: function link() {
...

{
   angular.module("icsm.clip", ['geo.draw']).directive('icsmInfoBbox', function () {
      ...
      function drawComplete(data) {
         ...
         var area = (clip.xMax - clip.xMin) * (clip.yMax - clip.yMin);
...
var GaStrategy = function (_BaseStrategy3) {
   ...
   function GaStrategy(http) {
      ...
      _this3.GA_METADATA_TEMPLATE = 'https://ecat.ga.gov.au/geonetwork/srv/eng/search#!${uuid}';
