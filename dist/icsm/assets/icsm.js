...

{
				...
				'geo.draw', 'geo.elevation', 'geo.geosearch', 'geo.map', 'geo.maphelper', 'geo.measure', 'placenames.search', 'placenames.config', 'placenames.summary', 'icsm.bounds', 'icsm.contributors', 'icsm.coverage', 'icsm.clip', 'icsm.elevation.point', 'icsm.glossary', 'icsm.help', 'icsm.imagery', 'icsm.panes', "icsm.parameters", 'icsm.point', 'icsm.products', 'icsm.preview', "icsm.side-panel",
				// Alternate list
				...
				.config(['$locationProvider', 'configServiceProvider', 'placenamesConfigServiceProvider', 'projectsServiceProvider', 'persistServiceProvider', 'versionServiceProvider', 'elevationPointServiceProvider', function ($locationProvider, configServiceProvider, placenamesConfigServiceProvider, projectsServiceProvider, persistServiceProvider, versionServiceProvider) {
            ...
            if (!state.layers) {
               configService.getConfig("map").then(function (config) {
                  ...
                  mapService.getMap().then(function (map) {
                     state.map = map;
                     state.lookup = {};
                     state.layers.forEach(function (element) {
...
angular.module("icsm.elevation.point", []).factory("elevationPointService", ['$q', 'configService', function ($q, configService) {
   var service = {
...
{
   var generateUrl = function generateUrl(data) {
      var index = data.file_url.lastIndexOf(".zip");
      return data.file_url.substr(0, index) + ".jpg";
   };

   angular.module("icsm.imagery", []).directive("launchImage", ["$rootScope", function ($rootScope) {
      return {
...
{
   angular.module("icsm.point", []).directive("pointElevation", ["elevationPointService", "mapService", function (elevationPointService, mapService) {
      return {
         restrict: "AE",
         ...
         link: function link(scope) {
            ...
            var handler = {
               ...
               searched: function searched(elevation) {
...
$templateCache.put("icsm/imagery/launch.html","<button type=\"button\" class=\"undecorated\" title=\"View preview of imagery\" ng-click=\"preview()\">\r\n	<i class=\"fa fa-lg fa-eye\"></i>\r\n</button>");
