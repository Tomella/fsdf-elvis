...
(function (angular) {
   ...
   angular.module("placenames.results.item", []).directive("placenamesResultsItem", ['placenamesResultsService', function (placenamesResultsService) {
      return {
      ...
      var service = {
         ...
         load: function load(id) {
            return this.config.then(function (_ref) {
               ...
               return proxy.get(esriTemplate.replace("${id}", id)).then(function (response) {
                  return response;
               });
            });
         },
      ...
      var service = {
         ...
         show: function show(what) {
            this.hide().then(function (map) {
               // split lng/lat string seperated by space, reverse to lat/lng, cooerce to numbers
               marker = L.marker(what.location.split(" ").reverse().map(function (str) {
                  return +str;
               })).addTo(map);
            });
         },
...
(function (angular) {
   ...
   angular.module("placenames.utils", []).filter("pnSplitBar", function () {
      return function (val) {
         var buffer = "";
         (val ? val : "").split("|").forEach(function (name, index, variants) {
            buffer += (index && index < variants.length - 1 ? "," : "") + " ";
            if (index && index === variants.length - 1) {
               buffer += "or ";
            }
