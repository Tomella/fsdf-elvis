...
(function (angular, L) {
   ...
   angular.module("common.featureinfo", []).directive("commonFeatureInfo", ['$http', '$log', 'featureInfoService', 'flashService', function ($http, $log, featureInfoService, flashService) {
      ...
      "LAYERS=public.NSW_100k_Index,public.5dem_ProjectsIndex" + "STYLES=&QUERY_LAYERS=public.5dem_ProjectsIndex&INFO_FORMAT=application%2Fjson&FEATURE_COUNT=100&X=${x}&Y=${y}";
      return {
         require: "^geoMap",
         restrict: "AE",
                     ...
                     if (response.data) {
                        layer = L.geoJson(response.data, {
                           ...
                           onEachFeature: function onEachFeature(feature, layer) {
                              layer.bindPopup("<strong>Map Title:</strong> <span title='Mapnumber: " + feature.properties.mapnumber + "'>" + feature.properties.maptitle + "</span><br/><strong>Status:</strong> " + feature.properties.status, { offset: [-10, -10] }).on('popupclose', function (e) {
                                 featureInfoService.removeLastLayer(map);
                              });
                           }
                        }).addTo(map).openPopup();
                        ...
                        L.popup().setLatLng(latlng).setContent('No data available at this location.').openOn(map);
                     }
