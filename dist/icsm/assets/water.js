...
{
   angular.module("water.regions", []).directive("waterRegions", ["$http", "configService", "mapService", function ($http, configService, mapService) {
      return {
         link: function link(scope) {
            var layer = void 0;
            mapService.getMap().then(function (map) {
               scope.map = map;
               configService.getConfig("regions").then(function (config) {
                  $http.get(config.regionsUrl).then(function (response) {
                     ...
                     var featureGroup = L.featureGroup(divisions, {
                        style: function style(feature) {
                           return {
                              color: "black",
                              fill: true,
